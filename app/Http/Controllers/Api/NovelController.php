...
class NovelController extends Controller
{
    ...
    public function writerDashboard(Request $request){
        ...
        $novels = Novel::where('user_id', $user->id)
            ->withCount('chapters') // total chapter per novel
            ->latest()
            ->limit(5)
            ->get();

        $stats = [
            'total_novels'   => Novel::where('user_id', $user->id)->count(),
            'total_chapters' => Chapter::whereHas('novel', function($q) use ($user) {
                                    $q->where('user_id', $user->id);
                                })->count(),
            'total_views'    => (int) Novel::where('user_id', $user->id)->sum('view_count'),
        ];
